...
Given(/^Launch application url "(.*?)"$/, (string) =>{
  ...
  // return client
  var client2 = client.page.pom()

  client2.url(string)
  .waitForElementVisible('body', 60000)
  .waitForElementVisible('@loginbtn', 60000,'Login button exist.')
...
Then(/^Next button is "(.*?)"$/, (BtnEnabled)=>{
      return client
      ...
      }else if(BtnEnabled=="enabled"){
        client.expect.element("//button[@id='next-btn']").to.be.enabled
        }
      })
      .waitForElementVisible("//button[@id='next-btn']",1000,'Next button is '+BtnEnabled)    
})


Then(/^All side menus are "(.*?)"$/, (menuEnabled)=>{
